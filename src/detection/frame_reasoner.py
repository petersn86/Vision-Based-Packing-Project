##############################################
# @Author: Joshua Tourigny
# @Contributor(s): Peter Nolan
# @Document: 'frame_reasoner.py'
#
# Description:
# This module uses Llama Vision to analyze an image crop
# from YOLO. It verifies the object against a master
# item_list.txt and returns a single, confirmed label.
#
##############################################
...
class FrameReasoner:
    ...
    def __init__(self, item_list_path: str, model_name: str = "llama3.2-vision"):
        ...
        self.item_list, self.item_list_text = self._load_item_list(item_list_path)
        ...
        try:
            ...
            print(f"[INFO] Successfully connected to Ollama and found model: {self.model_name}")
    ...
    #Load in the master list of items
    def _load_item_list(self, path: str) -> (Set[str], str):
        try:
            ...
            return items, item_list_text
        except FileNotFoundError:
            print(f"[ERROR] Item list file not found: {path}")
            return set(), ""
    ...
    def _build_prompt_template(self) -> str:
        ...
        Your response MUST be one of the following items from this master list:
        {self.item_list_text}

        If the object in the image is clearly not any of these items, or if it is a person or a cardboard box, respond with 'None'.
        Only respond with the single, best-matching item name from the list. Do not add any other text, explanation, or punctuation.
        Best match:"""

    #Cleans output and validates it with the master item list
    def _parse_response(self, response_text: str) -> Optional[str]:
        ...
        print(f"[WARNING] Llama response '{clean_response}' is not in the master item list. Discarding.") #A failed match results in being discarded, may need to be changed/adjusted
        return None

    #Using vision to refine cropped image/yolo label
    def refine_detection(self, image_crop: np.ndarray, yolo_label: str) -> Optional[str]:
